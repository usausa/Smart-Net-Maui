...
| SMU0005 | ❌ Error | The type containing the `[BindableProperty]` property is not derived from `BindableObject`, so `GetValue` and `SetValue` are not available | Derive the containing type from `BindableObject` |
